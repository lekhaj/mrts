...
public class Character : MonoBehaviour
{
    ...
    public Vector3 near_PlayerCastlePosition; // player castle position
    ...
    void Update()
    {
        ...
        // method to set the player to back to castle position
        if (CharacterManager.Retreat)
        {
            retreating = true;
            PlayerCastlePosition();
            agent.destination = near_PlayerCastlePosition;
        ...
        if (!goingToClickedPos)
        {
            ...
            {
                //if character is not moving to clicked position attack the castle ///----changes
                if (!goingToClickedPos && !retreating)
                {
                    if (!wizardSpawns)
                        agent.isStopped = false;
    ...
    //-------------------------------- Method which give player castle position ---------//

    public void PlayerCastlePosition()
    {

        //find neraby player castle
        GameObject[] Player_castle = GameObject.FindGameObjectsWithTag("Player Castle");

        //distance between player and nearedt Castle
        float closestDistance_Castle = Mathf.Infinity;

        foreach (GameObject potentialCastle in Player_castle)
        {
